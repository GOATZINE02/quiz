...
function updateNavButtons() {
  ...
  if (currentQuestionIndex === quizData.length - 1) {
    nextBtn.textContent = userAnswers.includes(null) ? 'Submit' : 'Submit';
...
nextBtn.addEventListener('click', () => {
  const isSubmit = nextBtn.textContent.toLowerCase() === 'submit';
  const isRestart = nextBtn.textContent.toLowerCase() === 'restart';

  if (isRestart) {
    restartQuiz();
  } else if (isSubmit && !userAnswers.includes(null)) {
    showResult();
